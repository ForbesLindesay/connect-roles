...
ConnectRoles.prototype.use = function() {
  if(arguments.length === 1) { this.use1(arguments[0]); }
  else if(arguments.length === 2) { this.use2(arguments[0], arguments[1]); }
  else if(arguments.length === 3) {
    this.use3(arguments[0], arguments[1], arguments[2]);
  } else {
    throw new Error('use can have 1, 2 or 3 arguments, not ' +
                    arguments.length);
  }
};
...
ConnectRoles.prototype.use1 = function(fn) {
  if(typeof fn !== 'function') {
    throw new TypeError('Expected fn to be of type function');
  }
...
ConnectRoles.prototype.use3 = function(action, path, fn) {
  ...
  if(typeof path !== 'string') {
    throw new Error('Expected path to be of type string');
  }
  var exp = pathToRegexp(path, keys);
  this.use2(action, function(req) {
    var pathToMatch = null;
    if(self.matchRelativePaths === true) { pathToMatch = req.url; }
    else { pathToMatch = req.app.path().replace(/\/$/, '') + req.path; }

    var match;
    if(match = exp.exec(pathToMatch)) {
      req = Object.create(req);
      req.params = Object.create(req.params || {});
      keys.forEach(function(key, i) { req.params[key.name] = match[i + 1]; });
      return fn(req);
    }
  });
};

ConnectRoles.prototype.or = routeTester2();
...
ConnectRoles.prototype.isAuthenticated = function() {
  ...
  var res = function(req, res, next) {
    if(typeof req.isAuthenticated !== 'function') { throw new Error(msg); }
...
function routeTester(verb) {
  return function(action) {
    function handle(onFail) {
      return function(req, res, next) {
        var act, grantAccess = false;
        if(action && action.constructor === Array) {
          for(var i=0; i<action.length; i++) {
            act = ert(req, action[i]);
            if(this.test(req, act)) { grantAccess = true; }
            else {
              grantAccess = false;
              break;
            }
...
function routeTester2() {
  return function(actionsSet) {
    function handle(onFail) {
      return function(req, res, next) {
        var grantAccessParent = false;
        var act, grantAccessChild = false;
        for(var j=0; j<actionsSet.length; j++) {
          var actions = actionsSet[j];
          if(actions && actions.constructor === Array) {
            for(var i=0; i<actions.length; i++) {
              act = ert(req, actions[i]);
              if(this.test(req, act)) { grantAccessChild = true; }
              else {
                grantAccessChild = false;
                break;
              }
            }
          } else {
            act = ert(req, actions);
            if(this.test(req, act)) { grantAccessChild = true; }
            else { grantAccessChild = false; }
          }
          if(grantAccessChild) {
            grantAccessParent = true;
            break;
          }
        }
        if(grantAccessParent) { next(); }
        else { onFail(req, res, next, act); }
      }.bind(this);
    }
    var failureHandler = this.failureHandler;
